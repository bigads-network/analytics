...
export default class User {
  ...
  static userExists: any = async (deviceId: any, appId: any): Promise<any> => {
    try {
      ...
      const result = await postgreDb
        .select()
        .from(users)
        .where(and(eq(users.deviceId, deviceId), eq(users.appId, appId)));
      return result[0];
    } catch (error: any) {
  ...
  static saveDetailsOwner: any = async (
    userId: any,
    secret_key: any,
    deviceId: any,
    saAddress: any
  ...
  static registerGame = async (
    ...
    try {
      ...
      const updatedgame = await postgreDb
        .update(games)
        .set({
          gameToken: Gametoken,
        })
  ...
  static getuserdetailsbyId = async (
    userId: any,
    gameId: any
  ): Promise<any> => {
    try {
  ...
  static checkGameExists = async (userId: any, gameID: any): Promise<any> => {
    try {
      return await postgreDb
        ...
        .from(games)
        .where(and(eq(games.createrId, userId), eq(games.id, gameID)));
    } catch (error: any) {
      throw new Error(error.message);
    }
  };
  ...
  static checkEvent = async (eventId: any, gameID: any): Promise<any> => {
    try {
      return await postgreDb
        ...
        .from(events)
        .where(and(eq(events.eventId, eventId), eq(events.gameId, gameID)));
    } catch (error: any) {
      throw new Error(error.message);
    }
  };
