...
const logSnapshot = (
  ...
  logger.info(
    `[${new Date().toISOString()}] Dashboard cache ${context} in ${
...
const refreshDashboardCache = async (reason: string) => {
  try {
    const snapshot = await DashboardCacheManager.refreshAll(reason);
    logSnapshot(`refresh (${reason})`, snapshot);
  } catch (error: any) {
    logger.error(
      `[${new Date().toISOString()}] Failed dashboard cache refresh (${reason}): ${
        error?.message || error
      }`,
    );
  }
